...
test('interpretPrompt proposes suggestions when asked for options', () => {
  const intent = interpretPrompt(createDocument(), 'Suggest cuts for filler words and pauses');
  assert.ok(intent);
  assert.equal(intent?.kind, 'suggest');
});

test('interpretPrompt applies structured cuts on direct remove requests', () => {
  const intent = interpretPrompt(createDocument(), 'Remove filler words and dead air');
  assert.ok(intent);
