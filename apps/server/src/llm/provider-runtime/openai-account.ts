...
import { accountAuthRoot } from '../../lib/paths.js';
...
const CODEX_DEFAULT_INSTRUCTIONS = `You are Codex, based on GPT-5. You are running as an editing agent inside Axcut on the user's computer.

Prefer acting with the available structured Axcut editing tools over prolonged deliberation. Timeline changes must go through those tools.`;
...
async function startCodexCallbackServer(state: string, verifier: string): Promise<boolean> {
  ...
  const server = http.createServer((req, res) => {
    ...
    res.end('<html><body><h3>OpenAI account connected.</h3><p>You can return to Axcut.</p></body></html>');
...
export async function beginCodexAuth(): Promise<CodexAuthChallenge> {
  ...
  url.searchParams.set('originator', 'axcut');
...
/** Path to the Axcut OpenAI account auth file. Override with AXCUT_CODEX_AUTH_PATH for tests. */
export function getCodexAuthPath(): string {
  return process.env.AXCUT_CODEX_AUTH_PATH || path.join(accountAuthRoot, 'openai-oauth.json');
}
...
export async function fetchOpenAiAccountModels(accessToken: string): Promise<string[]> {
  ...
  try {
    ...
    if (isModelDiscoveryAuthError(message)) {
      throw error;
    }
    ...
    console.warn(`[openai-account] Model discovery failed: ${message}.`);
    return [];
  }
}

function isModelDiscoveryAuthError(message: string): boolean {
...
export async function validateOpenAiAccountRuntime(modelId = OPENAI_ACCOUNT_DEFAULT_MODEL): Promise<{
  ...
  if (process.env.AXCUT_SKIP_CODEX_RUNTIME_VALIDATION === '1') {
...
export function createOpenAiAccountLanguageModel(
  ...
  return {
    ...
    async doStream(options) {
      ...
      const previousResponseId = readOptionalString(options.headers?.['x-axcut-previous-response-id']);
...
function ensureCodexInstallationId(): string {
  const installPath = path.join(accountAuthRoot, CODEX_INSTALLATION_ID_FILENAME);
  try {
