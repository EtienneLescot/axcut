...
from axcut_core.dsl import read_transcript_text
from axcut_core.models import Segment, Transcript
...
def _serialize_source_dsl(transcript: Transcript) -> str:
    ...
    for segment in transcript.segments:
        if segment.kind == "silence":
            ...
            continue
        lines.append(
            f"SEGMENT id={segment.id} start={segment.start:.3f} end={segment.end:.3f} text={json.dumps(segment.text, ensure_ascii=False)}"
        )
        for word in segment.words:
            lines.append(
                f"WORD id={word.id} segment={word.segment_id} start={word.start:.3f} end={word.end:.3f} text={json.dumps(word.text, ensure_ascii=False)}"
            )
